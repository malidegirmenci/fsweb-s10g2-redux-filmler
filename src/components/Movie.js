  ...
  return (
    <div className="bg-white rounded-md shadow flex-1">
      ...
      <div className="px-5 py-3 border-t border-zinc-200 flex justify-end gap-2">
        <button type="button" className="myButton bg-red-600 hover:bg-red-500" onClick={() => deletedMovie(movie.id)}>Sil</button>
        {displayFavorites ? <button className="myButton bg-blue-600 hover:bg-blue-500 " onClick={() => dispatch(addFavorite(movie))}>Favorilere ekle</button> : null}
      </div>
    </div>
  );
}
